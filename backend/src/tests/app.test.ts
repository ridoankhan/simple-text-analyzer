...
describe('Text API', () => {
  ...
  it('should update a text', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.content).toBe('The quick brown fox jumps over the lazy dog.');
  });
  ...
  it('should analyze a text', async () => {
    const createResponse = await request(app)
      .post('/api/texts')
      .send({ content: 'The quick brown fox jumps over the lazy dog.', createdBy: 'user1' });

    const analyzeResponse = await request(app).get(`/api/texts/${createResponse.body.id}/analyze`);
